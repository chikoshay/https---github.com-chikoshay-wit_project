...
def parent_list(my_hash, closest_wit):
    list_of_commits = []
    while my_hash != ROOT_ID:
        parent = find_parent(my_hash, closest_wit)
...
def copy_file(path2):
    ...
    staging_area = closest_wit / "staging_area"
    for dirpath, dirnames, _filenames in os.walk(closest_wit_tree, topdown=True):
